...
request(opt, function (err, res, body) {
    if (err) throw err;
    fs.writeFile('data/dataOmsk.txt', iconv.decode(body, 'utf-8'), (err) => {
        if (err) throw err;
...
function textIsEqual() {
    fs.readFile('data/etalonOmsk.txt', (err, data1) => {
        if (err) throw err;
        fs.readFile('data/dataOmsk.txt', (err, data2) => {
         if (err) throw err;
         if (data1.equals(data2)) {
            bot.sendMessage(chatId, 'ОМСК без изменений');
         } else {
            fs.writeFile('data/etalonOmsk.txt', data2, (err) => {
                if (err) throw err;
